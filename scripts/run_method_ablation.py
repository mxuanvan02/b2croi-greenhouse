#!/usr/bin/env python3
"""B2CRoI-H benchmark: hybrid mode-switch predictive RoI with service-floor guard.
...
def b2croi_score(xh,xt,a,ar,pi_bad,dual_f,dual_s,kind,counts,total_choices,h=4, q_s=None, q_f=None, q_e=0.0, variant="b2croi_h"):
    ...
    if variant=="h_no_mode":
        mode=0.0

    reliability=np.clip(psucc,0.25,1.0)
    if variant=="h_no_reliability":
    ...
    if variant!="h_no_constraint_bonus":
...
def choose(policy,krel,xt,xh,a,bw,ar,kind,pi_bad,dual_f,dual_s,counts,total_choices,q_s=0.0,q_f=None,q_e=0.0):
    ...
    elif policy in ['b2croi_h','h_no_mode','h_no_constraint_bonus','h_no_rmse_cost','h_no_reliability']:
...
def paired(raw, proposed='b2croi_h'):
...
def main():
    ...
    raw.to_csv(OUT/'b2croi_h_ablation_raw.csv',index=False)
    summ.to_csv(OUT/'b2croi_h_ablation_summary.csv',index=False)
    comp.to_csv(OUT/'b2croi_h_ablation_paired.csv',index=False)
    ...
    print('\nPaired B2CRoI deltas written to', OUT/'b2croi_h_ablation_paired.csv')
