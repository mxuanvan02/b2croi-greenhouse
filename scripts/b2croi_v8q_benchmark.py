...
def b2croi_score(xh,xt,a,ar,pi_bad,dual_f,dual_s,kind,counts,total_choices,h=4, q_s=None, q_f=None, q_e=0.0, variant="b2croi_v8q", tau_J=0.985, kappa_J=35):
...
def choose(policy,krel,xt,xh,a,bw,ar,kind,pi_bad,dual_f,dual_s,counts,total_choices,q_s=0.0,q_f=None,q_e=0.0, tau_J=0.985, kappa_J=35):
    ...
    elif policy=='b2croi_v8q':
        sc=b2croi_score(xh,xt,a,ar,pi_bad,dual_f,dual_s,kind,counts,total_choices,q_s=q_s,q_f=q_f,q_e=q_e,variant='b2croi_v8q',tau_J=tau_J,kappa_J=kappa_J)
...
def paired(raw, proposed='b2croi_v8q'):
...
def main():
    ...
    raw.to_csv(OUT/'b2croi_v8q_raw.csv',index=False)
    summ.to_csv(OUT/'b2croi_v8q_summary.csv',index=False)
    comp.to_csv(OUT/'b2croi_v8q_paired.csv',index=False)
    ...
    print('\nPaired B2CRoI deltas written to', OUT/'b2croi_v8q_paired.csv')
