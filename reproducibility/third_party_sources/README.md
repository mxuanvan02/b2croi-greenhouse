# Third-party baseline sources

No third-party baseline repository is currently vendored or executed by this reproducibility package.

If future work adds official baseline code, record the following before use:

1. method name;
2. repository URL;
3. exact commit/hash or release tag;
4. license and compatibility check;
5. local patch status, if any;
6. command used to reproduce the baseline result;
7. mapping from original outputs to reported metrics.

Do not clone or execute external code without reviewing the license, environment, and security implications.
